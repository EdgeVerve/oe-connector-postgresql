...
PostgreSQL.prototype.connect = function (callback) {
  ...
  self.pg.connect(function (err, client, done) {
    ...
    callback && checkDbExistenceAndCreate(err, client, self.settings, function (err, client) {
      if (err && !self.settings.ignoreErrors) {
        return callback(err, null);
      }
      self.createSearchPath(function (e) {
        // ignore error.
...
// Atul : when schema is not public, create search_path that includes public
// This is done because uuid-ossp extention could have been created on public schema
PostgreSQL.prototype.createSearchPath = function (cb) {
  if (this.settings.createSearchPath !== false && this.settings.schema && this.settings.schema.toLowerCase() !== 'public') {
    var s = `alter role "${this.settings.user}" in database "${this.settings.database}" set search_path to "${this.settings.schema}", public`;
    this.executeSQL(s, [], {}, function (err, results) {
      if (err) {
        console.log(err);
      }
      return cb(err);
  ...
  if (err && err.code === '3D000' && settings.database && (settings.enableDbCreation || settings.enableDbCreation === undefined)) { // database doesn't exists, trying to create a database.
    ...
    pool.connect(function (err, client, done) {
      ...
      client.query('CREATE DATABASE "' + dbToCreate + '"', function (err, res) {
        ...
        if (err && err.code != '42P04' && !settings.ignoreErrors) {
          callback(err, client);
        } else {
...
PostgreSQL.prototype.buildExpression = function (columnName, operator,
  ...
  if (operatorValue instanceof RegExp) {
    ...
    else if (operator === 'nilike') {
      this._notLike = true;
      this._ignoreCase = true;
    }
    val = operatorValue.source;
    operator = 'regexp';
  }
  switch (operator) {
    case 'like':
...
// Atul : Extracting model data from join data 
function prepareData(model, joinData) {
  var data = [];
  var pk = model.definition.idName() || 'id';
  ...
  for (var i = 0; i < joinData.length; ++i) {
    ...
    for (var k in item) {
      ...
      if (tableName.toLowerCase() !== modelName.toLowerCase()) {
        continue;
      }
      var columnName = k.split('$')[1];
      obj[columnName] = item[k];
    }
    var exists = data.find(function (item) {
      return item[pk.toLowerCase()] === obj[pk.toLowerCase()];
    });
    if (!exists) {
      data.push(obj);
    }
...
// Atul : this function is wrapper around execute()
// if there is joinData returned by join query present in options
// then it will not make database call but instead it returns model data extracted from joinData
PostgreSQL.prototype.executeWrapper = function (model, sql, params, options, cb) {
  if (options && options.joinData) {
    let data = prepareData(this.getModelDefinition(model).model, options.joinData);
    return process.nextTick(function () {
      return cb(null, data);
    });
  }
  return this.execute(sql, params, options, cb);
}


// Atul : This funtion builds join query for all models used in include clause
// you can have as much big include clause as supported by loopback
// {"useJoin" : true, "include" : [{ "relation" : "addressRel", "scope" : {"where" : { "or" : [{"city" : "aa" }, {"city" : "bb"}] }, "include" : "phoneRel" } }, {"relation" : "accountRel" ,  "scope" : {"where" : {"id" : {"neq" : "q"} } } } ]}
PostgreSQL.prototype.buildJoinQuery = function buildJoinQuery(jContext) {
  var self = this;
  var modelNames = Object.keys(jContext); //getAllModels(jContext[0]);
  var selectColumns = '';
  var fromTables = '';

  for (var i = 0; i < modelNames.length; ++i) {
    var modelName = modelNames[i];
    var temp = jContext[modelName]; //getJoinContext(jContext[0], modelName);
    var columns = this.buildColumnNames(temp.modelName, temp.filter);
    var realColumns = columns.split(",").map(function (item) {
      return self.escapeName(self.table(modelName)) + "." + item + ' as "' + self.table(modelName) + '$' + item.replace(/\"/g, '') + '"';
    }).join(",");
    var t = this.tableEscaped(temp.modelName);
    if (i > 0) {
      selectColumns += ", ";
      fromTables += ", ";
    }
  ...
  var firstLine = "SELECT " + selectColumns + " FROM " + fromTables;

  function delIdClause(filter, fk) {
    for (var k in filter) {
      if (k.toLowerCase() === fk.toLowerCase() && filter[k].inq && filter[k].inq[0] === '3888a315-f7fd-4e5d-bf20-ff3ae62e35af') {
        delete filter[k];
      }
      if (typeof (filter[k]) === 'object') {
        delIdClause(filter[k], fk);
      }
    }
  }
  ...
  var maxLimit = 100;
  //var mainTable = self.tableEscaped(jContext[0].modelName);

  for (var i = 1; i < modelNames.length; ++i) {
    var m = jContext[modelNames[i]]; //getJoinContext(jContext[0], modelNames[i]);
    var tableName = self.tableEscaped(m.modelName);
    var parentModel = self.getModelDefinition(m.parentModel).model;
    var parentTable = self.tableEscaped(parentModel.modelName);
    var fk = m.keyTo;

    var whereStmt;
    var pk = self.getModelDefinition(m.modelName).model.definition.idName() || 'id';
    pk = self.columnEscaped(m.modelName, pk);

    var parentPk = self.getModelDefinition(parentModel.modelName).model.definition.idName() || 'id';
    parentPk = self.columnEscaped(parentModel.modelName, parentPk);

    if (m.filter && m.filter.where) {
      /*Object.keys(m.filter.where).forEach(function(k){
        if(k.toLowerCase() === fk.toLowerCase()){
          delete m.filter.where[k];
        }
      });
      delete m.filter.where[fk];*/
      delIdClause(m.filter.where, fk);
      whereStmt = this.buildWhere(m.modelName, m.filter.where);
      if (whereStmt && whereStmt.sql) {
        whereStmt.sql = whereStmt.sql.replace("WHERE", "");
        whereStmt.sql = ` AND ( ${whereStmt.sql} )`;
      }
    }

    fk = self.columnEscaped(m.modelName, fk);

    var sql = ` 
      ${tableName}.${pk} in (
        SELECT id FROM (
          SELECT id, ROW_NUMBER() OVER (partition by ${fk}) r FROM ${tableName}
            WHERE ${tableName}.${fk} = ${parentTable}.${parentPk} 
            ${whereStmt.sql}
          ) q${i}
          WHERE 
            r < ${maxLimit}
            ) `
    innerQueries.push({ sql, params: whereStmt.params });
  }

  var paramsArray = [];

  var innerSQL = innerQueries.map(function (item) {
    paramsArray.push(item.params);
    return item.sql;
  }).join(" AND ");

  var params = Array.prototype.concat.apply([], paramsArray);
  var finalQuery = `${firstLine} WHERE ${innerSQL}`
  var sqlStmt = self.parameterize({ sql: finalQuery, params });
  return sqlStmt;
}
 ...
 */
PostgreSQL.prototype.innerall = function find(model, filter, options, cb, extra) {
  var self = this;

  // Atul : if filter is passed with 'useJoin' parameter
  // connector will try to execute loopback query using join
  // first join of model (mainmodel) falls under below if
  if (filter && filter.include && filter.useJoin && !extra) {
    var columns = self.buildColumnNames(model, filter);

    if (!options.join) {
      options.join = {}
      options.join[model] = { modelName: model, columns, filter };
    }
    var tempdata = {};
    var pk = self.getModelDefinition(model).model.definition.idName() || 'id';
    tempdata[pk] = "3888a315-f7fd-4e5d-bf20-ff3ae62e35af"

    // passing dummy tempdata instead of executing to actual database
    // this will ensure that all the observer hooks on models are executed as is 
    return self.getModelDefinition(model).model.include(
      [tempdata], filter.include, options, function (err, objs) {
        var stmt = self.buildJoinQuery(options.join);
        self.execute(stmt.sql, stmt.params, options, function (err, joinData) {
          if (err) {
            return cb(err);
          }
          delete options.join;
          self.innerall(model, filter, { joinData, notify: false }, function (err, data) {
            joinData = []; // trying to release memory
            return cb(err, data);
          }, true);
        })
      });
  ...
  // Atul : subsequent query falls in following if loop
  // this will help build options.join structure 
  // for each model it will have modelName, parentModel, key relation to parent model and entire clause(where, fields, sort etc)
  if (options && options.join) {
    var columns = this.buildColumnNames(model, filter);
    options.join[model] = Object.assign(options.join[model] || {}, { columns, filter, modelName: model });
    var pk = self.getModelDefinition(model).model.definition.idName() || 'id';
    var fk = options.join[model].keyTo;
    return process.nextTick(function () {
      let data = [];
      var tempdata = {};
      tempdata[pk.toLowerCase()] = '3888a315-f7fd-4e5d-bf20-ff3ae62e35af';
      tempdata[fk.toLowerCase()] = '3888a315-f7fd-4e5d-bf20-ff3ae62e35af';
      data = [tempdata];
      //let data =[{id: "a", employeeid : "a", "employeeaddressid" : "a"}];
  ...
  this.executeWrapper(model, stmt.sql, stmt.params, options, function (err, data) {
    if (err) {
      return cb(err, []);
    }
    if (filter.group) {
...
PostgreSQL.prototype.buildAggregationExpression = function (aggregationFn, columnName, operator,
  ...
  switch (operator) {
    ...
    case 'ilike':
      ...
      this._ignoreCase = true;
      return new ParameterizedSQL({
        sql: aggregationFn + columnName + " ILIKE ? ESCAPE '\\'",
        params: [operatorValue],
      });
    case 'nilike':
      this._notLike = true;
      this._ignoreCase = true;
      return new ParameterizedSQL({
        sql: aggregationFn + columnName + " NOT ILIKE ? ESCAPE '\\'",
        params: [operatorValue],
      });
    case 'regexp':
      if (!(operatorValue instanceof RegExp)) {
